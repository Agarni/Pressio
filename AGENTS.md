...
- The measurement display format (`13/8` vs `130/80`) is a persisted setting (`MeasurementDisplayFormat`) applied via the static `BloodPressureMeasurement.UseShorthandFormat`; change it in `SaveSettingsCommand` and reload measurements to refresh the rows.
