...
- In-app reminder notifications: `MainViewModel` polls enabled reminders every 20s (`CheckDueReminders`, via `Observable.Interval` on `RxApp.MainThreadScheduler`) and fires `ReminderBannerInteraction` when one is due; `MainView.axaml.cs` shows a small modal `Window`. Real OS/background notifications are not implemented (platform-specific follow-up).
