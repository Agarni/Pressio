...
- In-app reminder notifications: `MainViewModel` polls enabled reminders every 20s (`CheckDueReminders`, via `Observable.Interval` on `RxApp.MainThreadScheduler`) and shows an in-app overlay (`IsReminderNoticeVisible` + `ReminderNoticeMessage`, dismissed by `DismissReminderNoticeCommand`); `MainView` renders it as a topmost modal. Real OS/background notifications are not implemented (platform-specific follow-up).
