...
- **Sync (Fase S1):** `Patients`/`BloodPressureMeasurements`/`Reminders` carry `SyncId` (GUID) + `UpdatedAtUtc`, and `Measurements` also `PatientSyncId` — added/backfilled by the same `PRAGMA` migration. New rows get a fresh GUID. Snapshot DTOs live in `Pressio/Models/Sync/SyncModels.cs`, serialized by `SyncStore` (`pressio-sync.json`, camelCase + string enums); `SyncService.BuildLocalSnapshot()` composes the local source of truth. Design: `docs/SYNC.md`.
