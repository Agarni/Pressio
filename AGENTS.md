...
- CSV/PDF export lets the user pick where to save via a native "Save as" dialog: `MainViewModel.ExportFileInteraction` (ReactiveUI `Interaction`) is handled in `MainView.axaml.cs` with `Avalonia.Platform.Storage` (`FilePickerSaveOptions`). The last used directory is persisted (`LastExportDirectory` in `SettingsRepository`) and reused as the start location. The report obeys `ReportPeriod` (Todo/Últimos 7/30 dias/Período personalizado with date range) and is capped at the 30 most recent records (a truncation note is shown); the PDF header prints the exact period range.
